...
Cellular fields (IMEI, IMEI2, ICCID, IMSI, phone number) are **masked by default** (`SensitiveDataMasking.apply`, keeps only the last 4 characters, e.g. `••••9012`) since the app moved to public distribution. An eye-icon button in the Cellular card header (`@AppStorage("showSensitiveIdentifiers")`) reveals/hides them on demand, persisted per machine. This supersedes the earlier unmasked-by-default decision (which was justified at the time by strictly personal, single-device use).
